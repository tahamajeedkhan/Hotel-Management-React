...
// get all bookings
exports.getShowBookings = (req, res, next) => {
    var connectDB = mysql.createConnection({
        host: "localhost",
        user: "root",
        password: "",
    ...
    data = "SELECT * " +
        "FROM  bookingstatus " +
        "WHERE status = " + mysql.escape(value);
    ...
    connectDB.query(data, (err, result) => {
        if (err) throw err;
            ...
            res.render('admin/showBookings', { data: result });
        }
    });
};
        ...
        if (err) throw err;
            ...
            connectDB.query(data1, (err1, result1) => {
                if (err1) throw err1;
                    ...
                    return res.render('admin/index', { user: req.session.mail, msg: "", err: "", data: result1 });
                }
            })
        }
    })
...
exports.getSearchEmail = (req, res, next) => {
    res.render('admin/searchemail', { msg: "", err: "" })
}

//post request
exports.postSearchEmail = (req, res, next) => {
        ...
        data = "SELECT * " +
            ...
            "JOIN category c ON b.category=c.name " +
            "WHERE b.email = " + mysql.escape(req.body.email) +
            "AND b.category = " + mysql.escape(req.body.cat) +
            "AND b.type = " + mysql.escape(req.body.type) +
            "AND b.status = " + mysql.escape(value);

        connectDB.query(data, (err, result) => {
            if (err) throw err;
            else {
                return res.render('admin/checkout', { data: result, checkoutDate: req.body.checkoutDate, days: req.body.days });
            }
        })
...
exports.getCheckout = (req, res, next) => {
    ...
    connectDB.query(query, (err, result) => {
        if (err) {
            console.error(err);
            res.render('admin/searchemail', { msg: "", err: "No Data Found", data: null });
        } else {
            res.render('admin/checkout', { msg: "Successful Successful", err: "", data: result });
        }
    });
};
...
exports.postCheckout = (req, res, next) => {
    ...
    var value = 1;
    data = `INSERT INTO completedbookings (email,category,type,roomWant,roomcost,cdate,daystayed,totalbill) VALUES (
        ${mysql.escape(req.body.email)},
        ${mysql.escape(req.body.category)},
        ${mysql.escape(req.body.type)},
        ${mysql.escape(parseInt(req.body.roomWant))},
        ${mysql.escape(parseInt(req.body.roomcost))},
        ${mysql.escape(req.body.cdate)},
        ${mysql.escape(parseInt(req.body.days))},
        ${mysql.escape(parseInt(req.body.bill))}
      )`;
    console.log
    datat1 = 'DELETE FROM bookingstatus ' +
        ' WHERE  bid = ' + mysql.escape(req.body.bid);

    connectDB.query(data, (err, result) => {
        if (err) {
            console.error(err);
        } else {
            connectDB.query(datat1, (delerr, result1) => {
                if (delerr) {
                    console.error(delerr);
                } else {
                    res.render('admin/searchemail', { msg: "", err: "", data: result });
                }
            });
        }
        res.render('admin/searchemail', { msg: "Checkout Successful!", err: "", data: result });
    });
};
